...
| `sukuna_domain` | 宿儺 | 雙手手指緊密交疊，舉在嘴巴前面，呈手腕分開、指尖收攏的三角錐狀（雙手手勢，不是單手；⚠️ 手指遮擋嚴重，主要用「手腕距離明顯大於指尖距離 + 靠近嘴巴」的位置判斷） |
...
- `reverse_cross_palms`（反叉合掌）與 `sukuna_domain`（宿儺）都是雙手手指深度交疊、
  ...
  沒有真的檢查手指交叉/彎曲細節，穩定度比其他手勢差。兩者都是「雙手貼在一起舉高」，
  單純比手腕距離很容易搞混，所以用「手腕距離 vs. 指尖距離」的差異來區分：
  反叉合掌是手腕、指尖都貼在一起的平行合掌狀（兩個距離差不多），宿儺則是手腕分開、
  指尖收攏的三角錐狀（手腕距離明顯大於指尖距離），宿儺再額外加上「靠近嘴巴」的限制。
